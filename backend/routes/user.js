...
const bcrypt = require('bcrypt'); // 'bcryptjs'에서 'bcrypt'로 다시 수정했습니다.
...
router.post('/register', async (req, res) => {
    try {
        const { username, email, password, role, adminKey, name, nickname } = req.body;
        ...
        const existingUser = await User.findOne({ $or: [{ username }, { email }, { nickname }] });
        if (existingUser) {
